...
def main():
    ...
    if benchmark:
        ...
        if selenos_annot.shape[0] != sp4_prediction_subj.shape[0]:
            ...
            write(
                f"Selenoprotein families not present in query Selenoprofiles 4 prediction:\n\n{unpresent_sel}\n"
            )
    ...
    if benchmark:
        ...
        write(
            f"TP: {available_selenos.Transcript.isin(trans_candidates).sum()}/{len(available_selenos)}"
        )
    ...
    if not os.path.exists(path_fragmentation_outfile) or n_section < 3:
        ...
        write(
            f"> Table with best ORFs selected (for query and subject) per candidate --> {path_fragmentation_outfile}"
        )
    else:
    ...
    if True: #run_phase4 or benchmark:
        ...
        write(
            f"= After extending ORFs and removing duplicates, Nº of hits: {len(trans_candidates):,.0f}",
            how=colors['count'],
        )
        write(
            f"= After extending ORFs and removing duplicates, Nº of unique subject transcripts: {len(trans_candidates.unique()):,.0f}",
            how=colors['count'],
        )
        if benchmark:
            write(
    ...
    if True: #run_phase6 or benchmark:
        ...
        if benchmark:
            write(
                f"\nTP: {available_selenos.Transcript.isin(candidates_df.Chromosome).sum()}/{len(available_selenos)}:,.0f"
            )
    ...
    if not os.path.exists(path_selenocandidates_outfile) or n_section < 8:
        ...
        if benchmark:
            candidates_df = preprocessing_candidates(
                candidates_df, q_file, subj_file, sp4_subj_gff=sp4_subj_gff
            )
    ...
    write(
        f"= After final filter, Nº of candidates: {len(candidates_df)}",
        how=colors['count'],
    )
    write(
        f"= After final filter, Nº of unique subject transcripts: {len(candidates_df.Chromosome.unique())}",
        how=colors['count'],
    )
    ...
    write(
        f"> Output file with user-readable alignments created --> {path_pretty_outfile}"
    )
